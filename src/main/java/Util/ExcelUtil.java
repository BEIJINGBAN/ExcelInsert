...
//        }
    ...
    //分割后创建Excel
    public String easyExcel(String filePath, String fileName, List<Data> info, int maxSize){
        LinkedHashMap<String ,List<Data>> InfoMap = PartitionExcel(info,maxSize);
        if(InfoMap == null){
            return null;
        }
        for (Map.Entry<String,List<Data>> entry : InfoMap.entrySet()) {
            String ExcelPath = filePath + fileName + "_"+entry.getKey();
    ...
    public static class ExcelGenerator {
        ...
        //用于计算的信息
        private final Map<String, Object> shareData = new HashMap<>();
        ...
        //需要用来通用计算的数据
        public ExcelGenerator data(String key,Object value){
            shareData.put(key,value);
            return this;
        }

        public ExcelGenerator sheet(String sheetName, String headers,
                                    Function<Context,List<String[]>> dataSuplier){
            sheets.add(new SheetBuilder(sheetName,Arrays.asList(headers)),dataSuplier);
            return this;
        }
        public ExcelGenerator sheet(String sheetName,List<String[]> headerRows,
                                    Function<Context,List<String[]>> dataSupplier){
            sheets.add(new SheetBuilder(sheetName,headerRows,dataSupplier));
            return
        }

        public void save(String filePath)throws IOException{
            Context context = new Context(new HashMap<>(shareData));
